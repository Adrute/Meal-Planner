...
|---|---|---|
| `id` | uuid | PK |
| `title` | text | nombre de la quest |
| `frequency` | text | `daily` / `weekly` / `custom` / `annual` / `punctual` |
| `day_of_week` | text\|null | día habitual para quests semanales |
| `assigned_to` | text\|null | nombres separados por coma (multi-asignación) |
| `notes` | text\|null | notas libres |
| `custom_interval_days` | int\|null | intervalo en días para quests periódicas |
...
| `completed_by` | text\|null | nombre de quien realizó la quest |

### `task_week_assignments`
Sobreescritura semanal de día asignado y responsable (sin alterar los valores por defecto de la quest).
| campo | tipo | notas |
|---|---|---|
| `task_id` | uuid | FK household_tasks |
...
- Las quests periódicas solo aparecen si su próxima fecha de vencimiento cae en la semana actual o ya está vencida
...
|---|---|
| `TaskForm` | Formulario de creación/edición con todos los campos. Para quests periódicas permite especificar la fecha de la última realización al crearla. El botón de submit dice "Crear Misión" / "Guardar Cambios" |
...
Cada semana puede sobreescribir el día y el responsable de una quest sin modificar sus valores por defecto:
...
## Multi-asignación
El campo `assigned_to` almacena nombres separados por coma: `"Adrián, Paula"`. Los componentes convierten entre array y string con `parseNames` / `joinNames`. La opción `N/A` indica que la quest no está asignada a nadie concreto.
