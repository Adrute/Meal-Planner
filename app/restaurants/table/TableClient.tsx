      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1.5 p-2 bg-white rounded-2xl border border-slate-100 shadow-xl z-10 space-y-1 max-h-60 overflow-y-auto">
          {options.map(opt => {
            ...
            return (
              <button key={opt.id} onClick={() => handleToggle(opt.id)} className={`flex items-center gap-3 w-full p-2.5 rounded-lg text-left text-sm transition-colors ${isSelected ? 'bg-teal-50 text-teal-700 font-bold' : 'hover:bg-slate-50 text-slate-600'}`}>
                <div className={`w-4 h-4 rounded-md border flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'bg-teal-500 border-teal-500 text-white' : 'bg-white border-slate-300 text-transparent'}`}><Check size={12} strokeWidth={3} /></div>
                {opt.label}
              </button>
            )
          })}
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-3 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm items-center">
        <div className="relative flex-1 w-full">
          ...
          <input type="text" placeholder="Buscar por nombre o alérgeno..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-11 pr-4 py-3 rounded-2xl bg-slate-50 border-none outline-none focus:ring-2 focus:ring-teal-500/20 transition-all text-slate-700" />
        </div>
        <button onClick={() => setShowFilters(!showFilters)} className={`px-5 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 transition-colors ${hasActiveFilters ? 'bg-teal-500 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
      ...
      {showFilters && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xl space-y-6 animate-in slide-in-from-right duration-300">
          <div className="flex justify-between items-center pb-4 border-b border-slate-100">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><Filter size={20} className="text-teal-500" /> Panel de Filtros Avanzado</h3>
            <button onClick={() => setShowFilters(false)} className="text-slate-400 hover:text-teal-500 p-1 rounded-full hover:bg-teal-50 transition-colors"><X size={20} /></button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <button onClick={() => setFavFilter(!favFilter)} className={`p-3 rounded-2xl transition-colors text-sm font-bold flex items-center gap-2 justify-center border ${favFilter ? 'bg-lime-100 text-lime-500 border-lime-200' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
          ...
          <div className="pt-4 border-t border-slate-100 flex justify-end gap-2">
            {(statusFilters.length > 0 || typeFilters.length > 0 || tagFilters.length > 0 || favFilter) && (
              <button onClick={() => { setStatusFilters([]); setTypeFilters([]); setTagFilters([]); setFavFilter(false); }} className="px-4 py-2 rounded-xl text-sm font-medium text-slate-500 hover:text-teal-500">Limpiar todo</button>
            )}
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(rest => {
                ...
                if (isEditing) {
                  return (
                    <tr key={rest.id} className="bg-teal-50/30">
                      <td className="p-4 pl-6">
                        <input type="text" value={editData.name} onChange={e => setEditData({...editData, name: e.target.value})} className="w-full mb-2 px-3 py-2 rounded-lg border border-slate-200 text-sm font-bold focus:border-teal-500 outline-none" />
                        <select value={editData.food_type} onChange={e => setEditData({...editData, food_type: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs font-bold text-slate-600 focus:border-teal-500 outline-none">
                          {FOOD_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                      </td>
                      <td className="p-4 align-top">
                        <select value={editData.status} onChange={e => setEditData({...editData, status: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-700 focus:border-teal-500 outline-none">
                      ...
                      <td className="p-4 align-top">
                        <div className="flex flex-wrap gap-1 mb-2">
                          {editData.allergens?.map((tag: string) => (
                            <span key={tag} className="bg-slate-800 text-white px-2 py-1 rounded-md text-[10px] font-bold flex items-center gap-1">
                              {tag} <button onClick={() => setEditData({...editData, allergens: editData.allergens.filter((t:string) => t !== tag)})} className="hover:text-teal-400"><X size={10}/></button>
                            </span>
                          ))}
                        </div>
                        ...
                        }} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs focus:border-teal-500 outline-none" />
                      </td>
                ...
                return (
                  <tr key={rest.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="p-4 pl-6">
                      <div className="flex items-center gap-2">
                        <button onClick={() => toggleFav(rest)} disabled={isPending} className="focus:outline-none disabled:opacity-50" title={rest.is_favorite ? "Quitar favorito" : "Marcar como favorito"}>
                          <Star size={16} className={`transition-colors hover:scale-110 ${rest.is_favorite ? 'text-lime-400 fill-lime-400 drop-shadow-sm' : 'text-slate-200 hover:text-lime-400'}`} />
                        </button>
                    ...
                    <td className="p-4">
                      ...
                      {rest.status === 'pending' && <span className="bg-teal-50 text-teal-600 border border-teal-200 px-2.5 py-1 rounded-lg text-xs font-bold inline-flex items-center gap-1"><Clock size={14}/> Pendiente</span>}
                      {rest.status === 'doubtful' && <span className="bg-emerald-50 text-emerald-600 border border-emerald-200 px-2.5 py-1 rounded-lg text-xs font-bold inline-flex items-center gap-1"><Clock size={14}/> En duda</span>}
                      {rest.status === 'rejected' && <span className="bg-teal-50 text-teal-600 border border-teal-200 px-2.5 py-1 rounded-lg text-xs font-bold inline-flex items-center gap-1"><XCircle size={14}/> Descartado</span>}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => startEditing(rest)} className="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-400 p-2.5 rounded-xl hover:bg-teal-50 hover:text-teal-600 hover:border-teal-200 transition-all shadow-sm"><Edit3 size={16} /></button>
