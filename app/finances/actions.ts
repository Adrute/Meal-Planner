...
function autoCategory(conceptoOriginal: string, importe: number): string {
  ...
  if (/MERCADONA|LIDL|ALDI|CARREF|ALCAMPO|AHORRAMAS|PRIMAPRIX|FOODX/.test(u)) return 'Supermercado'
  if (/^COMPRA TARJ.*E\.S /.test(u) || /GASOLINERA|REPSOL|CEPSA|\bBP\b|SHELL/.test(u)) return 'Gasolina'
  if (/RESTAURANTE|JUST EAT|UBER EAT|GLOVO|BURGER|MCDONALDS|RODILLA|VIVE VIDA|APERITIVO|QUN YIN|QUN YING/.test(u)) return 'Restaurante'
  if (/NETFLIX|DISNEY|YOUTUBE|SPOTIFY|STARBUCKS|FNAC|CINE|TEATRO|KIWOKO|JUGUET|PIRATAS|TOBOGAN|MOVISTAR ARENA|HOMA TIENDA|PRIMARK MADRID|WONDERBLY|ARROYOMOLINOS H/.test(u)) return 'Ocio'
  if (/FCIA|FARMACIA/.test(u)) return 'Farmacia'
  if (/APARCAMIENTO|PARKING|METRO|RENFE|\bTAXI\b|\bUBER\b/.test(u)) return 'Transporte'
  if (/WELLHUB|DIGI SPAIN|PAYPAL \*NETFLIX|PAYPAL \*SPOTIFY|GOOGLE YOUTUBE|REVOLUT|ARBITRADE/.test(u)) return 'Suscripciones'
  if (/PRESTAMO|PRESTAMOS|HIPOTECA/.test(u)) return 'Préstamo'
  if (/TRANSFERENCIA|BIZUM|TRASPASO|ABONO/.test(u)) return 'Transferencia'
  if (/AGUA|TOTALENERGIES|VIVA AQUA|CANAL DE ISABEL|IBERDROLA|ENDESA/.test(u)) return 'Suministros'
  if (/SEGUROS|CASER|\bSEGURO\b/.test(u)) return 'Seguro'
  ...
  if (/IMPUESTO|TRIBUTOS|AYUNTAMIENTO/.test(u)) return 'Impuestos'
