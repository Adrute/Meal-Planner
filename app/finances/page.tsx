...
import FixedPatternsPanel from './fixed-patterns-panel'
...
export default async function FinancesPage({
  ...
  const [
    ...
    supabase.from('fixed_patterns').select('id, pattern, label').order('created_at', { ascending: false }),
  ])

  const fixedPatterns = fixedPatternsRaw ?? []
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 pb-24 md:pb-10 animate-in fade-in space-y-8">
      ...
      {/* GASTOS FIJOS */}
      {(!isAllTime && (totalFixed > 0 || fixedPatterns.length > 0)) && (
        <FixedPatternsPanel
          ...
          labelMonth={labelMonth(selectedMonth)}
        />
      )}
