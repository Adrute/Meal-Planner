...
import {
  assignMeal, removeMeal, deleteWeekPlan, assignMealByName,
  type HouseholdMember, type SchoolMenuItem,
} from './actions'
import {
...
export default function PlannerContainer({
  ...
  const handleRemoveWeek = async (weekStart: Date) => {
    ...
    if (confirm(`⚠️ ¿Borrar TODOS los planes de la semana del ${format(weekStart, 'd MMM')}?`)) {
      const res = await deleteWeekPlan(startStr, endStr)
      if (res.success) {
        setVisibleWeeks(prev => prev.filter(w => !isSameDay(w, weekStart)))
      } else {
        alert('Hubo un error al borrar la semana.')
      }
    }
  }
  ...
  return (
    <div className="space-y-12 animate-in fade-in pb-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex flex-col items-end gap-1">
            <button
              ...
              className="flex items-center gap-2 text-sm font-bold bg-violet-600 text-white hover:bg-violet-700 px-5 py-3 rounded-xl transition-all shadow-sm shadow-violet-200 active:scale-95"
            >
      ...
      {/* Panel de importación desde IA externa (Gemini u otra) */}
      {showImportPanel && (
        <div className="mb-4 p-5 bg-violet-50 rounded-2xl border border-violet-100">
          <p className="text-sm font-bold text-violet-800 mb-1">Importar menú generado por IA</p>
          <p className="text-xs text-violet-500 mb-3">
            Pega el JSON con las fechas exactas. Funciona para una semana o un mes completo.
            Las semanas nuevas aparecen automáticamente.
          </p>
          <textarea
            value={importText}
          ...
          <div className="flex justify-end gap-2 mt-3">
            <button
              ...
              className="text-xs font-bold text-slate-400 hover:text-slate-600 px-3 py-2 rounded-lg hover:bg-slate-100 transition-all"
            >
              Cancelar
            </button>
            <button
      ...
      {visibleWeeks.length === 0 && (
        <div className="text-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
          ...
          <button onClick={() => setIsModalOpen(true)} className="text-emerald-600 font-bold mt-2 hover:underline">
            Añadir una semana
          </button>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in">
          ...
          <div className="bg-white p-8 rounded-[2rem] shadow-2xl w-full max-w-sm relative z-10">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 p-2 bg-slate-50 hover:bg-slate-100 rounded-full transition-colors">
              <X size={20} />
            </button>
            <div className="mb-6">
            ...
            <input
              type="date"
              className="w-full p-4 border-2 border-slate-100 rounded-2xl mb-4 font-bold text-slate-700 focus:border-emerald-500 outline-none transition-colors cursor-pointer"
            ...
            <button
              ...
              className="w-full py-3 mb-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors shadow-lg disabled:opacity-50 flex justify-center items-center gap-2"
            >
...
function WeekBlock({
  startOfWeekDate, plan, recipes, householdMembers, schoolMenuItems,
  onSlotClick, onDeleteMeal, onRemoveWeek,
}: WeekBlockProps) {
  ...
  const handleGenerateDinners = async () => {
    ...
    try {
      const res = await fetch('/api/generate-meal-plan', {
        ...
        body: JSON.stringify({ weekDates, schoolMenu: weekSchoolMenu, recipes, members: householdMembers }),
      })

      const data = await res.json()
      if (!res.ok || data.error || !data.plan) {
        setGenerationResult(`Error: ${data.error ?? 'Respuesta inesperada de la IA'}`)
        return
      }
      ...
      setGenerationResult('Error de conexión')
    } finally {
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            >
              ...
              {isGenerating ? 'Generando...' : 'Generar Cenas IA'}
            </button>
            {generationResult && (
              <span className={`text-[10px] font-bold ${generationResult.startsWith('✓') ? 'text-emerald-600' : 'text-red-500'}`}>
                {generationResult}
              </span>
            )}
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs font-bold text-red-400 hover:text-red-600 hover:bg-red-50 px-4 py-2 rounded-xl border border-transparent hover:border-red-100 transition-all group disabled:opacity-50"
          >
            {isRemovingWeek ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
            {isRemovingWeek ? 'Borrando...' : 'Borrar semana'}
          </button>
        </div>
      </div>
              ...
              {schoolMenu && (
                <div className="mb-2 pb-2 border-b border-slate-50">
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest text-emerald-600">Cole</span>
                  </div>
                  <p className="text-[10px] text-slate-500 leading-tight">{schoolMenu.first_course}</p>
                  <p className="text-[10px] text-slate-400 leading-tight">{schoolMenu.second_course}</p>
                </div>
              )}

              <div className="space-y-3 flex-1 flex flex-col">
                {(['Almuerzo', 'Cena'] as const).map(type => {
                  const item = plan.find(p => p.day_date === dateStr && p.meal_type.toLowerCase() === type.toLowerCase())
          ...
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500 capitalize font-medium">
                {slot.type} • {format(slot.date, 'EEEE d', { locale: es })}
              </p>
            </div>
            <button onClick={onClose} className="p-2 bg-white hover:bg-slate-200 rounded-full shadow-sm border border-slate-100">
              <X size={18} />
            </button>
          </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map(r => (
              <button
                ...
                className="w-full text-left p-3 hover:bg-emerald-50 rounded-xl flex items-center gap-3 group transition-colors border border-transparent hover:border-emerald-100 disabled:opacity-50"
              >
