...
import { ChevronLeft, Clock, ChefHat, CheckCircle2, Pencil, Star, Tag, Trash2 } from 'lucide-react'
...
export default async function RecipeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const destroy = deleteRecipe.bind(null, id)
    ...
    <div className="max-w-3xl mx-auto p-4 md:p-8 pb-32">
      <nav className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <form action={destroy} onSubmit={(e) => { if (!confirm('¿Borrar esta receta?')) e.preventDefault() }}>
            <button className="flex items-center gap-1.5 text-sm font-bold text-red-400 hover:text-red-600 hover:bg-red-50 px-3 py-2 rounded-xl transition-colors">
